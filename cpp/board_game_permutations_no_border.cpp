// Enumerates all unique board permutations for a borderless board via BFS.
//
// Algorithm overview:
//   Same BFS structure as the combinations variant, but rotations are counted
//   as distinct boards — only height-offset normalization (zeroing by global
//   minimum) is used for dedup. This produces more unique results than combos.
//   No border constraint exists, so cells have no max height cap.
//
...
main(int argc, char* argv[])
{
  ...
  // For permutations, rotations are distinct so the result space is larger
  // than combinations.
    ...
    if (n_jobs > 1) {
      ...
      for (const std::string& filename : increment_files) {
        ...
        for (size_t i = 0; i < increments.size();
          ...
          // the canonical hash is just the zeroed board hash (no rotation
          // dedup).
